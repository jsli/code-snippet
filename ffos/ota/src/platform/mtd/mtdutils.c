...
static MtdState g_mtd_state = { NULL, // partitions
		0, // partitions_allocd
		-1 // partition_count
		};
...
int mtd_partition_info(const MtdPartition *partition, size_t *total_size,
		size_t *erase_size, size_t *write_size) {
	char mtddevname[32];
	...
	if (erase_size != NULL)
		*erase_size =
				(mtd_info.erasesize > 32 * 512) ? mtd_info.erasesize : 32 * 512;
